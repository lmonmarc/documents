...
En ce qui concerne le DSEL lui-même, il utilise plus particulièrement la notion d'\emph{expression template} \cite{Web4,Art21} qui est une façon de décrire des expressions grâce à la méta-programmation. C'est ainsi que sont stockés les stencils : il s'agit d'une expression binaire de coefficients. Les feuilles de l'arbre sont les coefficients tandis que les nœuds sont des sous-expressions de la racine (donc des morceaux du motif du stencil). Dans le cas du code ci-dessous présentant l'agrégation de quatre coefficients, l'arbre construit est en forme de peigne (à droite ou à gauche), comme dans la figure \ref{fig:tree1_coef}.
\begin{minted}{cpp}
...
{\caption{Exemple d'arbres en peigne d'un stencil à $4$ points formé en une seule fois.}\label{fig:tree1_coef}}
{\input{tikz3.tex}}
\end{figure}
Si le stencil est formé en deux temps, il est alors possible d'avoir un arbre équilibré comme dans l'exemple ci-dessous, avec la figure \ref{fig:tree2_coef}
\begin{minted}{cpp}
/* création d'un stencil à 4 points */
auto st1 = c1+c2;
auto st2 = c3+c4;
...
\begin{figure}[!h]
\floatbox[{\capbeside\thisfloatsetup{capbesideposition={right,center},capbesidewidth=4cm}}]{figure}[\FBwidth]
{\caption{Exemple d'arbre équilibré d'un stencil à $4$ points formé en plusieurs temps.}\label{fig:tree2_coef}}
{\input{tikz4.tex}}
\end{figure}
